...
export default function Calendar({ location }) {
      ...
      if (isTeacher) {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          const getTutorName = async () => {
            // await Axios.get(
            //   `api/calendar/tutorName/${response.data[i].TutorId}`
            // ).then((response) => {
            //   console.log(response.data.firstName);
            //   tutorName = response.data.firstName;
            // });
            eventArray.push({
              title: `You have tutoring with ${tutorName}`,
              start: response.data[i].start,
              end: response.data[i].end,
            });
          };
  ...
  const deleteEvent = (id) => {
    Axios.delete(`api/calendar/eventId/${id}`).then((response) => {
      ...
      // Axios.get(`api/calendar/eventinfo/${id}`).then((response) => {
      //   // this will get me the studentId and the tutorId
      //   console.log(response);
      // });
      ...
      // ).then((response) => {
      //   // this will get me the room number the tutor and student share
      // });
      // Axios.post(
      //   // this will post the message from the person that deleted the session
      //   `api/message/tutor:TutorId/student:StudentId/sender${userId}/room:room`,
      //   {
      //     message: `Your Tutoring Session on ${response.data.start} has been deleted.`,
      //   }
      // );
    });
  };
    ...
    <>
      ...
      <div className='columns'>
        ...
        <div className='container column'>
          {myCalendar === 'false' ? (
            <button
              className='button is-light is-info calendarButton'
